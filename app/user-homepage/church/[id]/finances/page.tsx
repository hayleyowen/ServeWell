...
export default function FinancesTrackingPage() {
    ...
    const saveToCloud = async () => {
        if (activeChart) {
            ...
            console.log("📤 Uploading file for", church, "-", pageType, ":", tabName);
            ...
            try {
                const response = await fetch("/api/church-files", {
                    method: "POST",
                    body: formData,
                });
    ...
    const deleteChart = async (chartId, tabName) => {
        ...
        try {
            const response = await fetch("/api/church-files", {
                ...
                body: JSON.stringify({ tab_name: tabName, church_id: churchID, page_type: pageType }),
            });
    ...
    const fetchStoredFiles = async (churchID, pageType) => {
        const apiURL = `/api/church-files?church_id=${encodeURIComponent(churchID)}&page_type=${encodeURIComponent(pageType)}`;
