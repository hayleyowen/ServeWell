...
export async function middleware(req: NextRequest) {
  ...
  // however, because the ministries are dynamic, we need to know which ministry they are assigned to
  // they will only be allowed to see the pages that are related to their ministry
  // for now, we will just redirect them to the homepage if they try to access any other page than their minisrtry's page
  try {}
  // first, we need to identify the user and their role
  ...
  // in the else case, the user is a ministry admin, and we need to check if they are trying to access their ministry's page
    ...
    // check if the user is trying to access any of their allowed pages
    if (req.nextUrl.pathname.startsWith(`/ministry/${ministryId}`) && ministryId == userMinistryId[0]?.ministryID) {
      return NextResponse.next()
    }
    // if not, we redirect them to the homepage
    else {return NextResponse.redirect(new URL('/', req.url))}
